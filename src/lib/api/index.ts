...
/** Корень API (без /docs — это страница Swagger, не JSON-эндпоинт). */
...
export async function fetchAllPrices(): Promise<ApiPricesResponse> {
  const url = `${BASE_URL}/api/v1/prices`;
  console.log('[API] fetchAllPrices →', url);
  const res = await fetch(url, { cache: 'no-store', headers: { Accept: 'application/json' } });
  console.log('[API] fetchAllPrices ←', res.status, res.headers.get('content-type'));
  const rawText = await res.text();
  console.log('[API] fetchAllPrices ← body (first 500):', rawText.slice(0, 500));
...
export async function fetchDrinkPrice(drinkId: string): Promise<ApiPriceItem> {
  const url = `${BASE_URL}/api/v1/prices/${encodeURIComponent(drinkId)}`;
  console.log('[API] fetchDrinkPrice →', url);
  const res = await fetch(url, { cache: 'no-store', headers: { Accept: 'application/json' } });
  console.log('[API] fetchDrinkPrice ←', res.status, res.headers.get('content-type'));
  const rawText = await res.text();
  if (!res.ok) {
...
export async function postSale(payload: ApiSaleRequest): Promise<void> {
  const url = `${BASE_URL}/api/v1/sales`;
...
export async function postSaleBatch(payload: ApiSaleBatchRequest): Promise<void> {
  const url = `${BASE_URL}/api/v1/sales/batch`;
